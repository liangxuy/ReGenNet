    ...
    def __init__(self, mode, model, parameters, dataiterator, device):
        ...
        with torch.no_grad():
            for databatch in tqdm(dataiterator, desc=f"Construct dataloader: {mode}.."):
                ...
                # TODO xuliang why not use translation here??
                # if translation and pose_rep != 'xyz':
                #     x = batch[feats][:, :-1]
                # else:
                #     x = batch[feats]
                x = batch[feats]
...
def evaluate(parameters, folder, checkpointname, epoch, niter, num_classes, model_path, num_person):
    ...
    if parameters['pose_rep'] == 'xyz':
        parameters["nfeats"] = 3 * num_person
        parameters["njoints"] = 25
    else:
        if parameters['body_model'] == 'smplx':
            parameters["nfeats"] = 6 * num_person
            parameters["njoints"] = 56
        elif parameters['body_model'] == 'smpl':
            parameters["nfeats"] = 6 * num_person
            parameters["njoints"] = 25
    ...
    # comment here by xuliang. 2022.10.29
    # recogparameters["pose_rep"] = "rot6d"
    # recogparameters["nfeats"] = 6

    # Action2motionEvaluation
    stgcnevaluation = STGCNEvaluation(dataname, recogparameters, model_path, device)
