...
class NewDataloader:
    def __init__(self, mode, model, dataiterator, device, jointstype):
        ...
        with torch.no_grad():
            for databatch in tqdm(dataiterator, desc=f"Construct dataloader: {mode}.."):
                if mode == "gen":
                    ...
                    batch = model.generate(classes, gendurations, jointstype=jointstype)
                    ...
                    batch["output_xyz"] = batch["x_xyz"] # [64, 18, 3, 60]
                elif mode == "rc":
...
def evaluate(parameters, folder, checkpointname, epoch, niter):
    ...
    try:
        for index, seed in enumerate(allseeds):
            ...
            motionloader = NewDataloader("gen", model, dataiterator, device, parameters["jointstype"])
            gt_motionloader = NewDataloader("gt", model, dataiterator, device, parameters["jointstype"])
            gt_motionloader2 = NewDataloader("gt", model, dataiterator2, device, parameters["jointstype"])
