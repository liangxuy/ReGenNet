...
from ..rotation2xyz import Rotation2xyz, Rotation2xyz_x

class CAE(nn.Module):
    def __init__(self, encoder, decoder, device, lambdas, latent_dim, outputxyz,
                 pose_rep, glob, glob_rot, translation, jointstype, vertstrans, body_model, **kwargs):
        super().__init__()
        ...
        if self.body_model == 'smpl':
            self.rotation2xyz = Rotation2xyz(device=self.device)
        elif self.body_model == 'smplx':
            self.rotation2xyz = Rotation2xyz_x(device=self.device)
    ...
    def generate_one(self, cls, duration, fact=1, xyz=False):
        ...
        output_xyz = self.rot2xyz(batch["output"], batch["mask"], fixrot=False)
    ...
    def generate(self, classes, durations, nspa=1,
        ...
        if self.outputxyz:
            batch["output_xyz"] = self.rot2xyz(batch["output"], batch["mask"], fixrot=fixrot)
        elif self.pose_rep == "xyz":
            batch["output_xyz"] = batch["output"]
