...
from utils.config import SMPL_MODEL_PATH, SMPLX_MODEL_PATH, JOINT_REGRESSOR_TRAIN_EXTRA
...
class SMPL(_SMPLLayer):
    ...
    def forward(self, *args, **kwargs):
        ...
        all_joints = torch.cat([smpl_output.joints, extra_joints], dim=1) # joints: 45
...
class SMPLX(_SMPLXLayer):
    def __init__(self, model_path=SMPLX_MODEL_PATH, **kwargs):
        ...
        smplx_indexes = np.arange(55)
        self.maps = {"smplx": smplx_indexes}
        
    def forward(self, *args, **kwargs):
        smplx_output = super(SMPLX, self).forward(*args, **kwargs)
        all_joints = smplx_output.joints # joints: 127
        output = {"vertices": smplx_output.vertices}
        for jointstype, indexes in self.maps.items():
            output[jointstype] = all_joints[:, indexes]
        return output
